...
function ENT:WaterPathFunc(area,from,ladder,elevator,length)
	if not IsValid(from) then return 0 else
		if not self.loco:IsAreaTraversable(area) then return -1 end
		
		local dist = 0
		
		if IsValid(ladder) then return -1
		elseif length > 0 then dist = length
		else dist = ( area:GetCenter() - from:GetCenter() ):GetLength()
		end
...
function ENT:PathFunc(area,from,ladder,elevator,length)
	if not IsValid(from) then return 0
	else
		if not self.loco:IsAreaTraversable(area) or area:IsUnderwater() then return -1 end
		...
		if deltaZ >= self.loco:GetStepHeight() then
			if deltaZ >= self.loco:GetMaxJumpHeight() then return -1 end
			cost = cost + 5 * dist
		elseif ( deltaZ < -self.loco:GetDeathDropHeight() ) then return -1 end
