...
function ENT:Think()
	...
	if self.CurAnim ~= 3 and self.CurAnim ~= 4 and self.CurAnim ~= 5 then
		...
		if pos:DistToSqr(PullCenter) <= v.PikPR then
			...
			timer.Create("expel"..self:EntIndex()..self.EXID,1.8,1,function()
				...
				for i=1,pikCount do
					...
					if not PikminCreateSproutServer(self,spawnPos) then
...
function ENT:Call(ply,call,send,color)
	...
	if send > 0 then
		...
		for _,v in ipairs(ents.FindByClass("pikmin")) do
			if not v.PikPly and v.Color == targetColor and v.Olimar == ply and (v.PikMdl.CurAnim == 1 or v.PikMdl.CurAnim == 2 or v.PikMdl.CurAnim == v.WingedIdle) then
				...
				if self.IsMasterOnion then
					table.insert(self.PikiListMaster[targetColor],v.Level)
				else
	...
	local list = self.IsMasterOnion and self.PikiListMaster[targetColor] or self.PikiList
	if call > 0 and #list >= call then
		timer.Create("call"..self:EntIndex(),0.1,1,function()
			for i=1,call do
				if PikminCreateServer(ply,{level=list[#list],color=targetColor}) then
...
function ENT:Use(activator,caller)
	...
	if activator:IsPlayer() then
		...
		activator:ConCommand("pikmin_omenu "..self:GetSkin().." "..#self.PikiList.." "..pikiCount..TooMany)
	end
end

function ENT:PreEntityCopy()
	local data = {Skin=self.Skin, PikiList=self.PikiList}
	if self.IsMasterOnion then
		data.PikiListMaster = self.PikiListMaster
	end
...
function ENT:PostEntityPaste(ply,ent,created)
	...
	if pikinfo then
		...
		if pikinfo.PikiListMaster then
			self.PikiListMaster = pikinfo.PikiListMaster
		end
		self.PullTable = {}
		self.EXID = 0
		...
		if self.IsMasterOnion then
			self.Color = 0
		elseif self.IsP3Onion then
			self.PikiColor = PikiOnionP3Colors[self:GetBodygroup(1)+1] or 1
