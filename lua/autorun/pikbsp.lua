...
BspContext = {
	...
	readEntities = function(self)
		...
		for i = 1, #entitiesText do
			...
			local targetname
			local model = nil
			local entityKeyValues = util.KeyValuesToTablePreserveOrder('"entities[' .. i .. ']"\x0A' .. entityText, false, true)
			...
			if entityKeyValues and #entityKeyValues ~= 0 then
				for j = #entityKeyValues, 1, -1 do
					...
					local key = string.lower(keyValue.Key)
